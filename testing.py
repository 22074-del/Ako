...
def open_quiz_page():
    # 1. Minimize the home page window immediately
    root.iconify()
    ...
    def next_question():
        ...
        if selected_answer.get() == "NONE":
            messagebox.showwarning("No Answer Selected", "Please choose an answer first.")
            return
...
def open_memory_page():
    ...
    memory_window.protocol("WM_DELETE_WINDOW", lambda: [root.deiconify(), memory_window.destroy()])
    tk.Label(memory_window, text="Welcome to the Memory Page!", font=("Arial", 18)).pack(pady=20)
...
def open_revision_page():
    ...
    revision_window.protocol("WM_DELETE_WINDOW", lambda: [root.deiconify(), revision_window.destroy()])
    tk.Label(revision_window, text="Welcome to the Revision Page!", font=("Arial", 18)).pack(pady=20)
...
def open_challenge_page():
    ...
    challenge_window.protocol("WM_DELETE_WINDOW", lambda: [root.deiconify(), challenge_window.destroy()])
    tk.Label(challenge_window, text="Welcome to the Challenge Page!", font=("Arial", 18)).pack(pady=20)
...
def home_page():
    ...
    status = tk.Label(root, text="Welcome to Kupu Quest!", font=("Arial", 50, "bold"))
    status.pack(pady=20)

    tk.Button(root, text="Quiz", command=open_quiz_page, width=50, height=5).pack(pady=20)
    tk.Button(root, text="Memory", command=open_memory_page, width=50, height=5).pack(pady=20)
    tk.Button(root, text="Revision", command=open_revision_page, width=50, height=5).pack(pady=20)
    tk.Button(root, text="Challenge", command=open_challenge_page, width=50, height=5).pack(pady=20)
